...
(function ()
{
    ...
    function Validator($injector)
    {
        ...
        var invalidFunc = function (element, validMessage, validation, scope, ctrl, attrs)
        {
            ...
            if (element.attr('no-validation-message') || !ctrl.$dirty)
            {
                messageElem.css('display', 'none');
            }
        ...
        var checkValidationGroup = function (scope, element, attrs, ctrl)
        {
            ...
            for (var i = 0, len = validationGroupElems.length; i < len; i++)
            {
                ...
                if (validationGroupElem.hasClass('ng-valid') && validationGroupElem[0] !== element[0])
                    return true;
            }
            return false;
        };
        ...
        var checkValidation = function (scope, element, attrs, ctrl, validation, value)
        {
            ...
            if (expression.constructor === Function)
            {
                return $q.all([$validationProvider.getExpression(validator)(value, scope, element, attrs, validatorParam)])
                  .then(function (data)
                  {
                      ...
                      {
                          return (!!data) ? valid.success() : valid.error();
                      } else if (data && data.length > 0 && data[0] && data[0].hasOwnProperty('isValid'))
                      {
                          return (!!data[0].isValid) ? valid.success() : valid.error();
                      }
            ...
            else if (expression.constructor === RegExp)
            {
                ...
                if (value !== undefined && value !== null && !$.isBlank(value)) //RH
                    ...
                    if (validationGroup)
                    {
                        // Whenever the element is invalid, we'll check whether one of the elements inside the its group valid or not.
                        // If there is a valid element, its invalid message won't be shown, Otherwise, shows its invalid message.
                        if (!checkValidationGroup(scope, element, attrs, ctrl))
                            valid.error();
                    }
        ...
        return {
            ...
            link: function (scope, element, attrs, ctrl)
            {
                ...
                }, function (value)
                {
                    ...
                    //        angular.element(document.querySelector('#' + (messageId || validationGroup))).html('');
                    //    else
                    //        $validationProvider.getMsgElement(element).html('');
                    //    return;
                    //}
                ...
                {
                    if (!value) // all hidden controls are valid
                    {
                        ctrl.$setValidity(ctrl.$name, true);
                    }
                    else
             ...
             */
                ...
                {
                    if (value) // all disabled controls are valid
                    {
                        ctrl.$setValidity(ctrl.$name, true);
                    }
                    else
